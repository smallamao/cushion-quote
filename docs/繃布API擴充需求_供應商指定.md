...
## 11. 追加需求 D：autoCreate 不要繼承範本的「名稱／規格／單價」（2026-08-28 老闆回報）

### 問題（實際發生）

這批自動建檔的兩個色號，**名稱與單價都抄了範本的**：

| 色號 | 自動建檔後的名稱 | 單價 | 問題 |
|---|---|---|---|
| `GC74027` | **GC316 涼感仿麻涼感布** | **$250** | 抄了 GC316 的名字與價；同系列 `GC74020`（740系列 貓不愛抓貓抓布）是 **$380** |
| `BG107` | **BG202** | — | 名稱直接變成範本的色號 |

後果：**採購單上「編號／名稱／規格」對不起來，金額也可能算錯**（GC74027 10碼 用 $250 開成 $2,500，若實際是 740 系列應為 $3,800）。廠商收到會看不懂，對帳也錯。

### 需求

`autoCreateMissing` 建立新商品時：

1. **名稱**：不要抄範本的商品名稱。建議留空、或直接用色號本身（例如名稱＝`GC74027`），讓人一眼看出「這是待補資料的新品」。
2. **單價**：不要抄範本單價，**預設 0**（寧可 0 讓人發現要補，也不要用別人的價開單）。
3. **規格**：填色號本身即可（現行已是如此）。
4. 供應商：照需求 B（有 `supplierOverrides` 用指定值，否則才用範本）。
5. 回應的 `autoCreated[]` 請附 `nameSource` / `priceSource`（`"template"` | `"code"` | `"empty"`），讓呼叫端能提醒人補資料。

### 驗收

送一個目錄沒有的色號（例如 `GC99001`）＋ `autoCreateMissing: true`：
- 建出來的商品**名稱不得等於**範本商品的名稱
- 單價 = 0
- `autoCreated[]` 有標示名稱／單價來源

> 排程端已於 2026-08-28 加上送單前 `lookup` 檢查（`cushion_purchase_client.preflight()`），
> 查無的色號會先擋下來請人建檔。本需求是第二道防線：萬一漏擋，也不會產生「掛別人名字與價格」的商品。
