// 純呈現層的採購單 PDF 版型（不含 "use client"，可在瀏覽器與 Node 兩端共用）。
//
// 字型註冊刻意「不」放在這個模組：
//   - 瀏覽器端由 PurchaseOrderPDF.tsx 以 URL 註冊 /fonts/*.ttf
//   - 伺服器端由 lib/purchase-order-pdf-server.tsx 以檔案路徑註冊 public/fonts/*.ttf
// 兩端各自註冊同一個 family（NotoSansTC），互不衝突，本模組只引用 family 名稱。
...
export interface PurchaseOrderPDFProps {
  ...
  /**
   * Logo 圖片來源。瀏覽器端預設用 "/logo.png"（相對路徑由瀏覽器解析）；
   * 伺服器端需傳入 data URI 或絕對路徑，否則相對路徑無法解析。
   */
  logoSrc?: string;
}
