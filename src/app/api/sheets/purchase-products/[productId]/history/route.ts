...
function productFromRow(row: string[]): PurchaseProduct {
  return {
    ...
    productName: row[3] ?? "",
    specification: row[4] ?? "",
    category: (row[5] as PurchaseProduct["category"]) ?? "面料",
    unit: (row[6] as PurchaseProduct["unit"]) ?? "碼",
    supplierId: row[7] ?? "",
    supplierName: row[8] ?? "",
    unitPrice: toNumber(row[9]),
    widthCm: toNumber(row[10]) || undefined,
    costPerCai: toNumber(row[11]) || undefined,
    listPricePerCai: toNumber(row[12]) || undefined,
    ...
    leadTimeDays: toNumber(row[20]) || undefined,
